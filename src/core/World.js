...
class World {
  ...
  _initScene() {
    ...
    const fieldGeometry = new THREE.BoxBufferGeometry(
      this.field.x,
      this.field.y,
      this.field.z,
    );
    const fieldMaterial = new THREE.MeshLambertMaterial({ color: 0xa0a0a0 });
    ...
    const obtacleMaterial = new THREE.MeshLambertMaterial({ color: 0x666666 });
    ...
    this.protectionMesh = new THREE.Mesh(
      protectionGeometry,
      protectionMaterial,
    );
    ...
    this.ui.continueButton.addEventListener(
      "click",
      this._onContinueButtonClick,
      false,
    );
    this.ui.restartButtonMenu.addEventListener("click", this._onRestart, false);
    this.ui.restartButtonComplete.addEventListener(
      "click",
      this._onRestart,
      false,
    );
    this.ui.restartButtonGameOver.addEventListener(
      "click",
      this._onRestart,
      false,
    );
  ...
  _createGuard() {
    ...
    const coreShieldDestroyed = this.assetManager.cloneAudio(
      "coreShieldDestroyed",
    );
  ...
  _checkPlayerCollision() {
    ...
    for (let i = 0, l = guards.length; i < l; i++) {
      ...
      if (squaredDistance <= range * range) {
        if (
          player.obb.intersectsBoundingSphere(guard.boundingSphere) === true
        ) {
          // dead
    ...
    for (let i = 0, l = pursuers.length; i < l; i++) {
      ...
      const squaredDistance = player.position.squaredDistanceTo(
        pursuer.position,
      );
      ...
      if (squaredDistance <= range * range) {
        if (
          player.obb.intersectsBoundingSphere(pursuer.boundingSphere) === true
        ) {
          // dead
    ...
    for (let i = 0, l = towers.length; i < l; i++) {
      ...
      if (squaredDistance <= range * range) {
        if (
          player.obb.intersectsBoundingSphere(tower.boundingSphere) === true
        ) {
          // dead
  ...
  _checkEnemyProjectileCollision(projectile) {
    ...
    for (let i = 0, l = obstacles.length; i < l; i++) {
      ...
      if (squaredDistance <= range * range) {
        ...
        if (
          obstacle.obb.intersectsBoundingSphere(projectile.boundingSphere) ===
          true
        ) {
    ...
    if (squaredDistance <= range * range) {
      if (
        player.obb.intersectsBoundingSphere(projectile.boundingSphere) === true
      ) {
  ...
  _checkPlayerProjectileCollision(playerProjectile) {
    ...
    for (let i = 0, l = guards.length; i < l; i++) {
      ...
      const squaredDistance = playerProjectile.position.squaredDistanceTo(
        guard.position,
      );
  ...
  _updateProjectileMeshes() {
    ...
    const enemyDestructibleProjectileCount =
    ...
    for (let i = 0; i < enemyDestructibleProjectileCount; i++) {
      ...
      this.enemyDestructibleProjectileMesh.setMatrixAt(
        i,
        projectile.worldMatrix,
      );
    }

    this.enemyDestructibleProjectileMesh.count =
      enemyDestructibleProjectileCount;
